...
def templateMatch(pixelData, queryImage, threshold=0):
		template = queryImage
		w, h = template.shape[::-1]
		res = cv.matchTemplate(pixelData, template, cv.TM_CCOEFF_NORMED)
...
waist = cv.imread(f"{argv[3]}/waist.png", cv.IMREAD_GRAYSCALE)
...
_,thresh = cv.threshold(img, 12, 255, cv.THRESH_BINARY)
thresh = cv.medianBlur(thresh, 5)
waistRect = templateMatch(thresh, waist)

shape = img.shape
isOnWaist = np.zeros(shape)
for i in range(shape[0]):
	for j in range(shape[1]):
		if isPointInsidePolygon((j,i), waistRect) is True:
			isOnWaist[i][j] = 1

df = pd.DataFrame()
pixelData = img.reshape(-1)
df["isOnWaist"] = isOnWaist.reshape(-1)

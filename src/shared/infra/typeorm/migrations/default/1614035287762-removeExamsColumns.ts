...
export class removeExamsColumns1614035287762 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query('alter table exams drop column equalizedImgLocation');
		await queryRunner.query('alter table exams drop column originalImgHistogramLocation');
		await queryRunner.query('alter table exams drop column equalizedImgHistogramLocation');
	}
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query('alter table exams add column equalizedImgLocation varchar');
		await queryRunner.query('alter table exams add column originalImgHistogramLocation varchar');
		await queryRunner.query('alter table exams add column equalizedImgHistogramLocation varchar');
	}
